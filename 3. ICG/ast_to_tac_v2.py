...
class ProgramConverter:
    ...
    def emit(self, op, arg1=None, arg2=None, result=None):
        ...
        if op in ['ASSIGN', 'ADD', 'SUB', 'MUL', 'DIV', '>', '<', '<=', '>=', '==', '!=', 'IF_FALSE']:
    ...
    def convert_expression(self, expr):
        ...
        if op == '=':
            ...
            if op == '+': self.emit('ADD', arg1_val, arg2_val, temp)
            elif op == '-': self.emit('SUB', arg1_val, arg2_val, temp)
            elif op == '*': self.emit('MUL', arg1_val, arg2_val, temp)
            elif op == '/': self.emit('DIV', arg1_val, arg2_val, temp)
            else: self.emit(op, arg1_val, arg2_val, temp) # For comparisons like <=, >= etc.
            return temp
        elif op == '++':
            ...
            false_branch_val = self.convert_expression(expr[3]) if len(expr) > 3 else None

            result_temp = self.new_temp() # Temporary variable to store the result of the if expression
            ...
            self.emit('IF_FALSE', cond_result, None, label_false) # If condition is false, jump to false branch

            # True branch
            self.emit('ASSIGN', true_branch_val, None, result_temp) # Assign true branch result to temp
            ...
            if false_branch_val is not None:
                self.emit('ASSIGN', false_branch_val, None, result_temp) # Assign false branch result to temp
    ...
    def convert_statement(self, stmt):
        ...
        if op == 'stmt':
            ...
            cond_result = self.convert_expression(actual_condition_expr) # Generates t_cond = i <= x
            self.emit('IF_FALSE', cond_result, None, label_loop_end) # If condition is false, exit loop
    ...
    def convert(self, s_expr_str):
        ...
        if parsed_ast and isinstance(parsed_ast[0], list) and parsed_ast[0][0] == 'main':
...
# --- Main execution block for file input and output ---
if __name__ == "__main__":
    ...
    icg_output_path = os.path.join(script_dir, 'icg_output.txt') # Output file in the same directory as this script
    ...
    try:
        ...
        if not s_expression_input:
            print(f"Error: {ast_output_path} is empty. Please ensure it contains the S-expression.")
            exit()
    ...
    # Open icg_output.txt to write the generated 3-address code
    try:
        with open(icg_output_path, 'w') as outfile:
            # Print the generated 3-address code in the desired format to the file
            for instruction in three_address_code:
                op = instruction[0]
                output_line = ""
                if op == 'ASSIGN':
                    output_line = f"{instruction[3]} = {instruction[1]}"
                elif op in ['ADD', 'SUB', 'MUL', 'DIV', '>', '<', '<=', '>=', '==', '!=']:
                    print_op = op
                    if op == 'ADD': print_op = '+'
                    elif op == 'SUB': print_op = '-'
                    elif op == 'MUL': print_op = '*'
                    elif op == 'DIV': print_op = '/'
                    output_line = f"{instruction[3]} = {instruction[1]} {print_op} {instruction[2]}"
                elif op == 'IF_FALSE':
                    output_line = f"ifFalse {instruction[1]} goto {instruction[3]}"
                elif op == 'GOTO':
                    output_line = f"goto {instruction[1]}"
                elif op == 'LABEL':
                    output_line = f"{instruction[1]}:"
                else:
                    output_line = f"UNHANDLED_INSTRUCTION: {instruction}"
