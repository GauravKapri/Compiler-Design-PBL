...
class ASTModuleGUI:
    def __init__(self, root):
        ...
        # Store the output for saving to file
        self.ast_output = ""
    ...
    def save_ast_output_to_file(self, output_content):
        """Save the AST output to ast_output.txt for ICG module"""
        try:
            output_file_path = os.path.join(self.script_dir, "ast_output.txt")
            with open(output_file_path, "w") as f:
                f.write(output_content)
            self.append_output(f"\nAST output saved to: {output_file_path}\n")
            return True
        except Exception as e:
            self.append_output(f"\nError saving AST output: {str(e)}\n")
            return False

    def parse_and_format_output(self, raw_output):
        """Parse the raw output and format it for ICG module"""
        lines = raw_output.strip().split('\n')
        
        # Find Symbol Table and Preorder Traversal sections
        symbol_table_lines = []
        preorder_lines = []
        ...
        for line in lines:
            if "Symbol Table" in line:
                ...
                in_preorder = False
                continue
            elif "Preorder Traversal" in line:
                in_symbol_table = False
                in_preorder = True
                continue
            
            if in_symbol_table and line.strip():
                symbol_table_lines.append(line)
            elif in_preorder and line.strip():
    ...
    def run_module(self):
        ...
        try:
            ...
            with open(cpp_file, "r") as input_file:
                ...
                raw_output = final_process.stdout
                self.append_output(raw_output)
                
                if final_process.stderr:
                    self.append_output(f"\n[stderr]\n{final_process.stderr}")
                
                # Parse and save the output for ICG module
                if raw_output.strip():
                    formatted_output = self.parse_and_format_output(raw_output)
                    if self.save_ast_output_to_file(formatted_output):
                        self.set_status("AST generated and saved successfully.")
                    else:
                        self.set_status("AST generated but failed to save output file.")
                else:
                    self.set_status("No AST output generated.")
